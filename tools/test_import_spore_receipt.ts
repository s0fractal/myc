...
const IMPORTER_PATH = new URL("./import_spore_receipt.ts", import.meta.url).pathname;
...
const VALID_RECEIPT = {
  ...
  record_hex: "53504f5200010001011e205bd70a84dce70b28c018ddbe253d1ef96557007816a7ecaa9c4609a2524ca10d1e20ed1c6b56db653a6d4e71d363f70b3dc4efa4b36b3f844663bd171e07350fdf991e20cf42e43aee73abbbfbcdec23fa8b2c66162ce579a160e8cbadfbcf4693bd138a",
...
Deno.test("MYC Importer - Invalid output_hash", async () => {
  const tampered = { ...VALID_RECEIPT, output_hash: "ff" + VALID_RECEIPT.output_hash.substring(2) };
  await assertRejects(
    () => runImporter(tampered),
    Error,
...
Deno.test("MYC Importer - Invalid record_hex (tampered payload)", async () => {
  const tampered = { ...VALID_RECEIPT, record_hex: VALID_RECEIPT.record_hex.substring(0, VALID_RECEIPT.record_hex.length - 2) + "ff" };
  await assertRejects(
    () => runImporter(tampered),
    Error,
...
Deno.test("MYC Importer - Invalid spore_id", async () => {
  const tampered = { ...VALID_RECEIPT, spore_id: "ff" + VALID_RECEIPT.spore_id.substring(2) };
  await assertRejects(
    () => runImporter(tampered),
    Error,
    "Invalid receipt: spore_id does not match record_hex",
